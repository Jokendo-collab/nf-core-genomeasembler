# nf-core/genomeasembler: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/genomeasembler/usage](https://nf-co.re/genomeasembler/usage)
...
```console
nextflow run nf-core/genomeasembler --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/genomeasembler
```
...
First, go to the [nf-core/genomeasembler releases page](https://github.com/nf-core/genomeasembler/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
